...
impl DfsScheduler {
    ...
    /// and an optional bound on the maximum depth to explore. A DFSScheduler can optionally
    /// allow random data to be generated by the test (using `shuttle::rand`). Enabling random
    /// data makes the DFS search incomplete, as it will not explore all possible values for
    /// the random choices. To ensure determinism, each execution will use the same sequence of
    /// random choices.
    pub fn new(max_iterations: Option<usize>, max_depth: Option<usize>, allow_random_data: bool) -> Self {
