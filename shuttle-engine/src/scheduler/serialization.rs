...
fn deserialize_fixed_width(mut bytes: &[u8]) -> Option<Schedule> {
    ...
    // The writer clamps the field to at least one bit, so a zero-width field is corrupt. It also has
    // to be rejected rather than merely producing a wrong answer: loading from an empty bit region
    // panics.
    if task_id_bits == 0 || task_id_bits > usize::BITS as usize {
        return None;
    }
...
mod test {
    ...
    fn malformed_input_is_rejected() {
        ...
        // A fixed-width header claiming a zero-bit task id field. Reading a task from it would load
        // from an empty bit region, which panics rather than returning garbage.
        assert_eq!(deserialize_schedule("9100010086"), None, "zero-width task id field");
