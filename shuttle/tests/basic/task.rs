...
pub struct SignatureSubscriber {
    pub signatures: SigCounterMap,
    pub static_create_locations: SigCounterMap,
}
...
pub fn check_n_same_signatures(signatures: &SigCounterMap, expected_count: usize) {
...
pub fn check_n_different_signatures(signatures: &SigCounterMap, expected_count: usize) {
    let signatures = signatures.lock().unwrap();
    trace!("Total signatures captured: {}", signatures.len());
