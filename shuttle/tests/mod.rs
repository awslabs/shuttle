    ...
    S: Scheduler + UnwindSafe + 'static,
{
    ...
    let mut schedules = std::fs::read_dir(dir)
        ...
        .collect::<Vec<_>>();
    schedules.sort();
    // One failure means one schedule. A failing execution reaches the failure reporting path twice
    // (from the panic hook and again from the runtime once the panic has unwound), and reporting both
    // would leave the reader guessing which file to replay.
    assert_eq!(
        schedules.len(),
        1,
