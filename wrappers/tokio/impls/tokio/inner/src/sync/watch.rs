//
// Note: This code is mostly copied directly from the tokio sources, version 1.19.2
//
...
mod state {
    ...
    impl AtomicState {
        ...
        pub(super) fn load(&self) -> StateSnapshot {
            StateSnapshot(self.0.load(Ordering::SeqCst))
        }
...
impl<T> Receiver<T> {
    fn from_shared(version: Version, shared: Arc<Shared<T>>) -> Self {
        ...
        // not memory access.
        shared.ref_count_rx.fetch_add(1, Ordering::SeqCst);
    ...
    pub fn borrow(&self) -> Ref<'_, T> {
        let inner = self.shared.value.blocking_read();
        Ref { inner }
    }
    ...
    /// }
    /// ```
    pub async fn wait_for(&mut self, mut f: impl FnMut(&T) -> bool) -> Result<Ref<'_, T>, error::RecvError> {
    ...
    ///
    /// ```text
    /// let (tx, rx) = crate::sync::watch::channel(true);
    /// let rx2 = rx.clone();
    /// assert!(rx.same_channel(&rx2));
    ///
    /// let (tx3, rx3) = crate::sync::watch::channel(true);
...
fn maybe_changed<T>(shared: &Shared<T>, version: &mut Version) -> Option<Result<(), error::RecvError>> {
    ...
    tracing::trace!("maybe_changed with version {:?} and {:?}", *version, new_version);
...
impl<T> Drop for Receiver<T> {
    fn drop(&mut self) {
        ...
        // not memory access.
        tracing::trace!(
            "dropping {:p} with count {:?}",
            self,
            self.shared.ref_count_rx.load(Ordering::SeqCst)
        );
        if 1 == self.shared.ref_count_rx.fetch_sub(1, Ordering::SeqCst) {
...
impl<T> Sender<T> {
    ...
    pub fn send(&self, value: T) -> Result<(), error::SendError<T>> {
        tracing::trace!(
            "watch {:p} send value with receiver count {:?}",
            &self.shared,
            self.receiver_count()
        );
    ...
    {
        {
            ...
            let mut lock = self.shared.value.blocking_write();
    ...
    ///
    /// ```text
    /// let (tx, rx) = crate::sync::watch::channel(());
    /// assert!(!tx.is_closed());
    ///
    /// drop(rx);
    /// assert!(tx.is_closed());
...
mod tests {
    ...
    fn watch_spurious_wakeup() {
        shuttle::check_random(
            || {
                let (send, mut recv) = watch::channel(0i32);
    ...
    /// This test exposes an issue when [`shuttle::sync::RwLock`] is used in the [`watch`] implementation.
    ///
    /// The issue is that the concurrent evaluation of select guards can break internal invariants of `RwLock`.
    /// Concretely, the read in `borrow_and_update` puts the caller on the `RwLock` waiter list. But then
    /// the other select arm can wake up the caller while the watch send holds the write lock. This breaks
    /// the expectation in `RwLock` that waiters are only woken up when they are granted the lock, and
    /// results in the following panic.
    ///
    /// ```text
    /// resumed a waiting Read thread while the lock was in state Write(main-thread(2))
    /// ```
    ///
    /// See commit `751a0433`, which switched from [`shuttle::sync::RwLock`] to [`crate::sync::RwLock`].
    #[test]
    fn watch_with_select() {
        use crate::sync::mpsc;
                ...
                future::block_on(async move {
                    ...
                    let h1 = future::spawn(async move {
                        tokio::select! {
                            biased;
                            _ = mpsc_rx.recv() => {}
                            _ = async { watch_rx.borrow_and_update() } => {}
                        }
                    });
                    ...
                    let h3 = future::spawn(async move {
                        let _ = mpsc_tx.send(());
                    });

                    futures::future::join_all([h1, h2, h3]).await;
                });
            },
            10_000,
        );
