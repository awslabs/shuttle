...
impl<T> sealed::FromStreamPriv<T> for Box<[T]> {
    ...
    fn finalize(_: sealed::Internal, collection: &mut Vec<T>) -> Box<[T]> {
        <Vec<T> as sealed::FromStreamPriv<T>>::finalize(sealed::Internal, collection).into_boxed_slice()
    }
}
    ...
    U: FromStream<T>,
{
    ...
    fn initialize(_: sealed::Internal, lower: usize, upper: Option<usize>) -> Result<U::InternalCollection, E> {
        Ok(U::initialize(sealed::Internal, lower, upper))
    }

    fn extend(_: sealed::Internal, collection: &mut Self::InternalCollection, item: Result<T, E>) -> bool {
...
pub(crate) mod sealed {
    ...
    pub trait FromStreamPriv<T> {
        ...
        fn initialize(internal: Internal, lower: usize, upper: Option<usize>) -> Self::InternalCollection;
