use shuttle_tokio_impl_inner_stream::{self as stream, Stream, StreamExt};
...
fn size_overflow() {
    ...
    impl shuttle_tokio_impl_inner_stream::Stream for Monster {
        type Item = ();
        fn poll_next(self: std::pin::Pin<&mut Self>, _cx: &mut std::task::Context<'_>) -> std::task::Poll<Option<()>> {
            panic!()
        }
