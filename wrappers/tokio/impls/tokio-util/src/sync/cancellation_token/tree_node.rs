...
pub(crate) fn is_cancelled(node: &Arc<TreeNode>) -> bool {
    node.inner.blocking_lock().is_cancelled
}

/// Creates a child node
pub(crate) fn child_node(parent: &Arc<TreeNode>) -> Arc<TreeNode> {
    let mut locked_parent = parent.inner.blocking_lock();
...
fn disconnect_children(node: &mut Inner) {
    for child in std::mem::take(&mut node.children) {
        let mut locked_child = child.inner.blocking_lock();
...
{
    let mut locked_node = node.inner.blocking_lock();
    ...
    loop {
        ...
        let locked_parent = match potential_parent.inner.try_lock() {
            ...
            // NOTE: Uncomment the `TryLockError::WouldBlock` part here and uncomment below if we swap back to std::sync
            Err(_ /* TryLockError::WouldBlock */) => {
                drop(locked_node);
                // Deadlock safety:
                //
                ...
                let locked_parent = potential_parent.inner.blocking_lock();
                locked_node = node.inner.blocking_lock();
                locked_parent
            } // NOTE: Uncomment this and the `TryLockError::WouldBlock` part above if we swap back to std::sync
              /*
...
fn move_children_to_parent(node: &mut Inner, parent: &mut Inner) {
    ...
    for child in std::mem::take(&mut node.children) {
        {
            let mut child_locked = child.inner.blocking_lock();
...
fn remove_child(parent: &mut Inner, mut node: MutexGuard<'_, Inner>) {
    ...
    if parent.children.len() == pos + 1 {
        ...
        replacement_child.inner.blocking_lock().parent_idx = pos;
...
pub(crate) fn cancel(node: &Arc<TreeNode>) {
    ...
    while let Some(child) = locked_node.children.pop() {
        ...
        while let Some(grandchild) = locked_child.children.pop() {
            ...
            let mut locked_grandchild = grandchild.inner.blocking_lock();
