...
use shuttle_tokio_impl_inner_util::sync::{CancellationToken, WaitForCancellationFuture};
use tokio::pin;
...
async fn cancel_token() {
    ...
    assert_eq!(Poll::Pending, wait_fut.as_mut().poll(&mut Context::from_waker(&waker)));
...
#[tokio::test]
async fn cancel_token_owned_drop_test() {
    let (waker, wake_counter) = new_count_waker();
    let token = CancellationToken::new();
    ...
    assert_eq!(Poll::Pending, future.as_mut().poll(&mut Context::from_waker(&waker)));
    assert_eq!(wake_counter, 0);
...
#[tokio::test]
async fn cancel_child_token_through_parent() {
    let (waker, wake_counter) = new_count_waker();
    let token = CancellationToken::new();
    ...
    assert_eq!(Poll::Pending, child_fut.as_mut().poll(&mut Context::from_waker(&waker)));
    assert_eq!(
        Poll::Pending,
...
#[tokio::test]
async fn cancel_grandchild_token_through_parent_if_child_was_dropped() {
    let (waker, wake_counter) = new_count_waker();
    let token = CancellationToken::new();
...
#[tokio::test]
async fn cancel_child_token_without_parent() {
    let (waker, wake_counter) = new_count_waker();
    let token = CancellationToken::new();
...
#[tokio::test]
async fn create_child_token_after_parent_was_cancelled() {
...
#[tokio::test]
async fn drop_multiple_child_tokens() {
...
#[tokio::test]
async fn cancel_only_all_descendants() {
    // ARRANGE
    let (waker, wake_counter) = new_count_waker();
    ...
    assert_eq!(Poll::Pending, fut.as_mut().poll(&mut Context::from_waker(&waker)));
    assert_eq!(
        Poll::Pending,
    ...
    assert_eq!(
        Poll::Pending,
        grandchild_fut.as_mut().poll(&mut Context::from_waker(&waker))
    );
    assert_eq!(
        Poll::Pending,
        grandchild2_fut.as_mut().poll(&mut Context::from_waker(&waker))
    );
    assert_eq!(
        Poll::Pending,
        great_grandchild_fut.as_mut().poll(&mut Context::from_waker(&waker))
    );
    assert_eq!(wake_counter, 0);
    ...
    assert_eq!(Poll::Ready(()), fut.as_mut().poll(&mut Context::from_waker(&waker)));
    assert_eq!(
        Poll::Ready(()),
        child1_fut.as_mut().poll(&mut Context::from_waker(&waker))
    );
...
#[tokio::test]
async fn drop_parent_before_child_tokens() {
    let token = CancellationToken::new();
...
#[tokio::test]
async fn run_until_cancelled_test() {
...

    {
        ...
        assert_eq!(Poll::Ready(None), fut.as_mut().poll(&mut Context::from_waker(&waker)));
    }
