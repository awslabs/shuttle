...
use shuttle_tokio_impl_inner_util::task::TaskTracker;
...
#[tokio::test]
async fn notify_immediately_on_reopen() {
    let tracker = TaskTracker::new();
    tracker.close();
...
#[tokio::test]
async fn notify_on_close_reopen() {
    let tracker = TaskTracker::new();
...
#[tokio::test]
async fn notify_on_last_task_respawn() {
    let tracker = TaskTracker::new();
    tracker.close();
    let token = tracker.token();
...
#[tokio::test]
async fn no_notify_on_respawn_if_open() {
    let tracker = TaskTracker::new();
    let token = tracker.token();
